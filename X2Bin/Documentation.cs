...
X2Bin --build build.txt --output directory/to/dict
    ...
    each line in build.txt is a set of args, will output combined dicts and scripts to --output directory

    Use --commands to see a list of commands
        --schema-help to see the manual
";


    private const string Commands = @"
X2Bin --schema schema.yaml --input file/or/folder --output folder [--mode xml|yaml|json]
    [--singleton] [-extension .thing] [--int 7bit|int8|int16|int32] [--float single|double] 
    [--string 7bit|int8|int16|int32|nullterm] [--encoding ascii|utf-8|utf-16|...]
    [--trim-string aggressive|passive|none] [--trim-code cstyle|aggresive|passive|none]
    [--enum path/to/assembly.dll] [--csharpscript path/to/assembly.dll]
